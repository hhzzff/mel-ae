...
def val_loop(batch, state, accel):
    ...
    return {
        "loss": state.mel_loss(out["mel_out"], out["mel_in"]),
        "mel/loss": state.mel_loss(out["mel_out"], out["mel_in"]),
    }
...
def train_loop(state, batch, accel, lambdas):
    ...
    with accel.autocast():
        output["adv/disc_loss"] = state.gan_loss.discriminator_loss(out["mel_out"], out["mel_in"])
    ...
    with accel.autocast():
        output["mel/loss"] = state.mel_loss(out["mel_out"], out["mel_in"])
        (
            output["adv/gen_loss"],
...
def save_samples(state, val_idx, writer):
    ...
    for k, v in audio_dict.items():
        for nb in range(v.batch_size):
            print(f"saving {k}/sample_{nb}.wav to TensorBoard")
...
def train(
    ...
    val_loop = tracker.track("val", None)(val_loop)
