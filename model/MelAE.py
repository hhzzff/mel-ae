...
class MelAE(nn.Module):
    ...
    def noise(self, z):
        if self.training:
            ...
            SNR = torch.rand(z.shape[0], 1, 1).to(z) * (1 - self.min_SNR) + self.min_SNR
            noisy_z = SNR * z + noise * (1 - SNR)
            return noisy_z
        else:
            return z
